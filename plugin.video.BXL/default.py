...
addon = xbmcaddon.Addon('plugin.video.BXL')
...
def menu_donation():
    ...
    ret = dialog.select('Doação para Addon', ['[B]Conta Brasil[/B]','','[B]Caixa Economica Federal[/B]','[B]Agencia: 0160[/B]','[B]Conta: 00033882-5[/B]','','[B]Conta Belgica[/B]', '','[B]BE58 3631 6038 3779[/B]','[B]ID: ID 05968563 56[/B]'])


def getSources():
        #menu_donation()
        addDir('[B][COLOR red]DOAÇÂO PARA ADDON [/COLOR][/B][B][CLIQUE AQUI][/B]','Search Plugins',1000,icon,FANART,'','','','')
        try:
            if os.path.exists(favorites) == True:
