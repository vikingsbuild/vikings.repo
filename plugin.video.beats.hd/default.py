...
addon = xbmcaddon.Addon('plugin.video.beats.hd')
...
alive_source = os.path.join(profile, 'beats_source')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.beats.hd-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(beats.hd, code error - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(beats.hd, server error. - "+str(e.reason)+",10000,"+icon+")")

def SKindex():
    addDir('[B][COLOR lime]| Favourites[/COLOR][/B]','[B][COLOR lime]| Favourites[/COLOR][/B]',4,'https://i.imgur.com/VVlKyOl.png',FANART,'','','','')
    getData(Base,FANART)
...
def getSources():
        try:
            if os.path.exists(favorites) == True:
                addDir('[B][COLOR lime]| Favourites[/COLOR][/B]','[B][COLOR lime]| Favourites[/COLOR][/B]',4,'https://i.imgur.com/VVlKyOl.png' , os.path.join(home, 'fanart.gif'),FANART,'','','','')
            if os.path.exists(beats_source)==True:
                sources = json.loads(open(beats_source,"r").read())
...
def getSearchData(url,icon, data=None):
    ...
    keyboard.setHeading("[COLOR white][B]Beats[COLOR lime].[COLOR white]HD Search[/B][/COLOR]")
    keyboard.setDefault('')
    keyboard.doModal()
    if keyboard.isConfirmed():
        ...
        xbmcgui.Dialog().ok('[COLOR white][B]Beats[COLOR lime].[COLOR white]HD[/B][/COLOR]', '[COLOR white][B]Blank Searches are not allowed.[/B][/COLOR]')
        quit()
    fanart=''
    dontLink=False
    os.path.join(home, 'resources', 'fanart.gif')
...
def getFavorites():
        if os.path.exists(favorites):
            ...
            for i in items:
                ...
                addDir('[COLOR red][B][ No Favourite Beats Added Yet ][/B][/COLOR]','','','','','','','','','fav')
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':           
                ...
                contextMenu.append(('[B][COLOR blue]Remove Beat Favourite[/COLOR][/B]','XBMC.RunPlugin(%s?mode=6&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
            if not name in FAV:
                contextMenu.append(('[B][COLOR lime]Add to Beats Favourites[/COLOR][/B]','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#                print 'setting as folder in link'
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('[B][COLOR blue]Remove Beat Favourite[/COLOR][/B]','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('[B][COLOR lime]Add to Beats Favourites[/COLOR][/B]','XBMC.RunPlugin(%s)' %fav_params))                                                                                
            liz.addContextMenuItems(contextMenu)
        try:
...
params=get_params()
Base = 'http://frenchdj.atspace.tv/Beats/Beats.HD.xml'
url=None
name=None
mode=None
playlist=None
