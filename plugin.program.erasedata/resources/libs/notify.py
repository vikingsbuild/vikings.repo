...
def autoConfig(msg='', TxtColor='0xFFFFFFFF', Font='font12', BorderWidth=10):
	class MyWindow(xbmcgui.WindowDialog):
		...
		def __init__(self,msg='',L=0,T=0,W=1280,H=720,TxtColor='0xFFFFFFFF',Font='font12',BorderWidth=10):
			...
			header = '[COLOR %s]Assistente de Configuracoes Avancadas[/COLOR]' % (COLOR2)
...
def firstRun():
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.msg   = "Atualmente, Nenhuma Build Instalada a Partir de %s.\n\nSelecione 'Build Menu' para instalar uma Build ou 'Ignorar' para nunca mais ver essa mensagem.\n\nObrigado por escolher %s." % (ADDONTITLE, ADDONTITLE)
		...
		def doIgnore(self):
			self.close()
			wiz.log("[First Run] [Usuario Selecionado: Ignorar o BUILD MENU] [Next Check: %s]" % str(NEXTCHECK), xbmc.LOGNOTICE)
			wiz.setS('lastbuildcheck', str(NEXTCHECK))
...
def notification(msg='', test=False):
	class MyWindow(xbmcgui.WindowXMLDialog):
		...
		def doRemindMeLater(self):
			...
			wiz.log("[Notification] NotifyID %s Lembre-me mais Tarde" % wiz.getS('noteid'), xbmc.LOGNOTICE)
			self.close()

		def doDismiss(self):
			if not test == True:
				wiz.setS("notedismiss","true")
			wiz.log("[Notification] NotifyID %s Entendi" % wiz.getS('noteid'), xbmc.LOGNOTICE)
			self.close()
...
def updateWindow(name='Testing Window', current='1.0', new='1.1', icon=ICON, fanart=FANART):
	class MyWindow(xbmcgui.WindowXMLDialog):
		...
		def doNormalInstall(self):
			wiz.log("[Check Updates] [Installed Version: %s] [Current Version: %s] [User Selected: Instalacao Normal]" % (BUILDVERSION, LATESTVERSION), xbmc.LOGNOTICE)
			wiz.log("[Check Updates] [Next Check: %s]" % str(NEXTCHECK), xbmc.LOGNOTICE)
			wiz.setS('lastbuildcheck', str(NEXTCHECK))
			self.close()
