...
addon_data_dir = xbmc.translatePath("special://home/addons/plugin.video.gospeltv/data").decode("utf-8")
...
def Categories():
	...
	for item in list:
		...
		AddDir("[COLOR yellow][B]{0}[/B][/COLOR]".format(name) ,item["url"].encode("utf-8"), mode, image.encode("utf-8"), logos.encode("utf-8"), index=i, cacheMin=cacheMin)
		i += 1
...
def Playthis(name, url, iconimage=None):
        toca = "plugin://plugin.video.playthis/?mode=play&player=false&path="+url+""
        PlayUrl(name, toca, iconimage=None)
