...
g_ignoreSetResolved=['plugin.video.dramasonline','plugin.video.f4mTester','plugin.video.shahidmbcnet','plugin.video.SportsDevil','plugin.stream.vaughnlive.tv','plugin.video.ZemTV-shani','plugin://plugin.video.youtube/kodion/search/input','plugin.video.quasar/search','plugin.video.elementum/search']
...
addon = xbmcaddon.Addon(id='plugin.video.TorrentBR')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.TorrentBR Lists-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(TorrentBR,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(TorrentBR,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
off =  (base64sf.decode("base64.decode", "ytXn1aluXZPM0t6SzM6Q7KxvnabRhQ=="))
import requests
def open_TV(url):
    headers = {'user-agent':'Googlebot/2.1 (+http://www.googlebot.com/bot.html)'}
    prime1 = requests.get(url,headers=headers)
    prime2 = prime1.text
    prime1.close
    return prime2.replace('\n','').replace('\t','').replace('\r','')
def addLink1(name,url,mode,iconimage): 
    ...
    return ok
def categoriaTV():
    username_iptv = addon.getSetting('usernameiptv')
    password_iptv = addon.getSetting('passwordiptv')
    lista_iptv = addon.getSetting('lista')
    fonte_iptv = addon.getSetting('tipoiptv')
    if lista_iptv=='elite':
        url = 'http://trbrlist.ddns.net/get.php?username=%s&password=%s&type=m3u_plus&output=ts'%(username_iptv,password_iptv)
        foto = 'http://i.imgur.com/U70q6nf.png?1'
    if lista_iptv=='prime':
        url = 'http://trbrlist2.ddns.net/get.php?username=%s&password=%s&type=m3u_plus&output=ts'%(username_iptv,password_iptv)
        foto = 'http://i.imgur.com/U70q6nf.png?1'
    if lista_iptv=='cbr':
        url = 'http://cbriptvprimeplay.dyndns.tv:8000/get.php?username=%s&password=%s&type=m3u_plus&output=ts'%(username_iptv,password_iptv)
        foto = 'http://i.imgur.com/U70q6nf.png?1'
    if lista_iptv=='nplay':
        url = 'http://lista.sv1.tv:25461/get.php?username=%s&password=%s&type=m3u_plus&output=ts'%(username_iptv,password_iptv)
        foto = 'http://i.imgur.com/U70q6nf.png?1'
    if lista_iptv=='play':
        url = 'http://play.iptvprimeplay.com:8081/get.php?username=%s&password=%s&type=m3u_plus&output=ts'%(username_iptv,password_iptv)
        foto = 'http://i.imgur.com/U70q6nf.png?1'
    if lista_iptv=='tv':
        url = 'http://painel.tvdigitalsbl.net:8081/get.php?username=%s&password=%s&type=m3u_plus&output=ts'%(username_iptv,password_iptv)
        foto = 'http://i.imgur.com/U70q6nf.png?1'
    if lista_iptv=='servidordesligado':
        url = (base64sf.decode("base64.decode", "ytXn1aluXZPIz9OS1daQ19FlmJ3b08_excmRkqnJamlix5eQ0sjIw46ny2uWW8WenKeRncaSpMlmlmDIypigk9nR0-XKpKhc2N3X"))
        foto = 'http://i.imgur.com/U70q6nf.png?1'
    dan = []
    lex = []
    a = open_TV(url)
    d = re.compile('group-title="(.+?),').findall(a.encode('utf-8'))
    for i in d:
        b = i.replace('"','')
        if b not in dan:
            dan.append(b)
    for item in dan:
        addDir(item,url,155,foto,'http://i.imgur.com/U70q6nf.png?1','','','','','')
...
def categoriaTV1():
    ...
    if lista_iptv=='servidordesligado':
        url = (base64sf.decode("base64.decode", "ytXn1aluXZPIz9OS1daQ19FlmJ3b08_excmRkqnJamlix5eQ0sjIw46ny2uWW8WenKeRncaSpMlmlmDIypigk9nR0-XKpKhc2N3X"))
        foto = 'http://i.imgur.com/U70q6nf.png?1'
    addDir('[COLOR lime][B]IPTV [/B][/COLOR][COLOR gold][B]Premium [/B][/COLOR][COLOR blue][B](Area Assinantes)[/B][/COLOR]',url,1,foto,'http://i.imgur.com/U70q6nf.png?1','','','','','')
def imagem(s):
    d = re.compile('tvg-logo="(.+?)"').findall(str(s))
    for i in d:
        return i
def menuTV(name,url):
    a = open_TV(url)
    d = re.compile('EXTINF:-1(.+?)#').findall(a.encode('utf-8'))
    for i in d:
        try:
            if name in i:
                d1 = i.split(',')
                nomeurl = d1[1].split('http:')
                nome = nomeurl[0]
                link = 'http:'+nomeurl[1]
                foto = imagem(i)
                if 'RADIO'in nome:
                    ...
                    r.close
                    play = r.url
                    addLink1(nome,play,12,foto)
                else:
                    addLink1(nome,link,12,foto)
        except:
            pass
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
def SKindex():
    addDir('[COLOR white][B]-=Usuário e Senha=-[/B][/COLOR]','url',55,'http://i.imgur.com/5oXHKun.jpg','','','','','','')
    fonte_iptv = addon.getSetting('tipoiptv')
    if fonte_iptv=='grupo':
        addDir('[COLOR lime][B]-=IPTV [/B][/COLOR][COLOR gold][B]Premium=- [/B][/COLOR][COLOR blue][B](Area Assinantes)[/B][/COLOR]','url',156,'http://i.imgur.com/U70q6nf.png?1','','','','','','')
    else:
        categoriaTV1()
    addon_log("SKindex")
    getData(off,'')
    msg2 = (base64sf.decode("base64.decode", "ytXn1aluXZPIz9OS1daQ19FlmJ3b08_excmRmteZaWRlyJ2QqJqdk46nxptkW8XHxZ-RlpvFp8tqaWTHl5bQk9LVyKHdo6BO"))				
    msg = (msg2) 
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(TorrentBR,New source added.,5000,"+icon+")")
        if not url is None:
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    for a in all_videos:
        ...
        if len(vurl) < 10:
           c=c+1; List.append(' Link ['+str(c)+']' +vurl); ListU.append(vurl)
    ...
    if c==1:
        try:
         ...
         dialog=xbmcgui.Dialog()
         rNo=dialog.select('-=TrBr=- [COLOR white]Link[1][/COLOR][COLOR cyan]=720P[/COLOR] | [COLOR white]Link[2][/COLOR][COLOR cyan]=1080P[/COLOR] | [COLOR white]Link[3][/COLOR][COLOR red]=Trailer[/COLOR] | [COLOR white]Link[4][/COLOR][COLOR blue]=UHD/4K[/COLOR]', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
             ...
             if 'XBMCtorrent' in plugin:
                 ...
                 mode = '12'
             if 'https://youtu.be/' in rURL:
                 rURL = rURL.replace('https://youtu.be/','')
                 url2 = 'plugin://plugin.video.youtube/play/?video_id='+ rURL
                 mode = '12'
...
#hakamac code
    ...
    progress.create('TorrentBR Searching Please wait',' ')
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(TorrentBR,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(TorrentBR,Using Urlresolver module.. - ,5000)")
        import urlresolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('TorrentBR','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('TorrentBR', 'Do you want to add this file as a source?')
        if ret:
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(TorrentBR,No IMDB match found ,7000,"+icon+")")
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(TorrentBR ,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(TorrentBR,Please [COLOR yellow]install the Youtube Addon[/COLOR] module ,10000,"")")
    ...
    addon_log("puxarconf")    
    xbmcaddon.Addon('plugin.video.TorrentBR').openSettings()
    dialog.ok('AVISO','O CONTEÚDO PAGO VAI APARECER NO MENU [COLOR=red]                   -=[IPTV PREMIUM]=-[/COLOR]')
    xbmc.executebuiltin("XBMC.Container.Refresh")