...
#### LINK DO ARQUIVO DO MENU PRINCIPAL NO HOST OU NO PASTEBIN ################
url_menu = 'https://brunoflix.tk/brunoflix/base-entrada/entrada-categorias.xml'
##############################################################################
#### NOME DE IDENTIFICAÇÃO DO ADDON, EXEMPLO:  plugin.video.testando  
#### OBS: ELE É O MESMO NOME DA PASTA E DO  addon id  EM addon.xml ###########
addon_id = 'plugin.video.brunoflix'
##############################################################################
####  BOAS VINDAS DO ADDON  ##################################################
welcome_message = '[B][COLOR white]||||||| SEJA BEM-VINDO AO BRUNOFLIX |||||||[/COLOR][/B]'
##############################################################################
####  LINK DO MENU BOAS VINDAS ###############################################
## OBS: POR PADRÃO JÁ TEM UM MENU EM BRANCO PARA NÃO TER ERRO AO CLICAR
welcome_link = 'https://brunoflix.tk/bemvindo.xml'
##############################################################################
###  DESCRIÇÃO DO ADDON ######################################################
### OBS: PARA SALTAR UMA LINHA UTILIZE \n ####################################
welcome_description = 'O BrunoFlix é Um Plugin de vídeo para o KODI que agrega Links de reprodução de Vídeos de vários tipos, sendo eles. Filmes, Series, Animes, Etc. parecido com a Netflix.\n\n[B][COLOR white]Versão: [/COLOR][COLOR magenta]1.0.0[/COLOR][/B]\n\n[B][COLOR yellow]Doações:[/COLOR][/B] [COLOR violet][/COLOR]'
##############################################################################
...
def SKindex():
    ...
    #### MENUS
    #### EXEMPLO DE MENU
    ### addDir('NOME DO MENU','LINK-DO-MENU',1,'LINK-DO-THUMBNAIL' ,  FANART,'DESCRIÇÃO','','','')
    addDir(welcome_message,welcome_link,1,icon,FANART,welcome_description,'','','')
    #addDir('[B][COLOR lime]●[/COLOR][COLOR white] FAVORITOS[/COLOR][/B]','Favoritos',4,'https://cdn.pixabay.com/photo/2014/03/25/16/54/star-297563_960_720.png' ,  FANART,'Menu de Favoritos dos Canais ou Filmes Adicionados','','','')
    addDir('[B][COLOR lime]●[/COLOR][COLOR white] FAVORITOS[/COLOR][/B]','Favoritos',4,favorites2,FANART,'Menu dos canais e filmes favoritos adicionados pelo usuario','','','')
    getData(url_menu,'')
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False):
        ...
        if showcontext:
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remover dos Favoritos do Addon','XBMC.RunPlugin(%s?mode=6&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
									
         
	
                contextMenu.append(('Adicionar aos Favoritos do Addon','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remover dos Favoritos do Addon','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Adicionar aos Favoritos do Addon','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
