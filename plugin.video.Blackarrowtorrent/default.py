...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.omegakts Lists-%s]: %s" %(addon_version, string))
        
lista = os.path.join(xbmc.translatePath("special://home/addons/plugin.video.Blackarrowtorrent/lista.xml").decode("utf-8"))
tube  = os.path.join(xbmc.translatePath("special://home/addons/plugin.video.Blackarrowtorrent/tube.xml").decode("utf-8"))

play  = os.path.join(xbmc.translatePath("special://home/addons/plugin.video.Blackarrowtorrent/play.m3u").decode("utf-8"))
mag  = os.path.join(xbmc.translatePath("special://home/addons/plugin.video.Blackarrowtorrent/mag.xml").decode("utf-8"))
web  = os.path.join(xbmc.translatePath("special://home/addons/webinterface.kratos/index.html").decode("utf-8"))
playlistv = os.path.join(xbmc.translatePath("special://home/addons/plugin.video.Blackarrowtorrent/playlistv.xml").decode("utf-8"))
vaddon = os.path.join(xbmc.translatePath("special://home/addons/plugin.video.Blackarrowtorrent/vaddon.xml").decode("utf-8"))

m3u = os.path.join(xbmc.translatePath("special://home/addons/plugin.video.Blackarrowtorrent/lista.m3u").decode("utf-8"))

webi  = os.path.join(xbmc.translatePath("special://home/addons/webinterface.kratos").decode("utf-8"))
...
def virtual_addon():
    ...
    linkva = dialog.input('[COLOR yellow]Cole o Codigo Base64 do Seu Virtual Addon[/COLOR]','---', type=xbmcgui.INPUT_ALPHANUM)
    faddon = base64.b64decode(linkva)
    f = open(vaddon, 'a')
    f.write(faddon)


def link_virtualaddon():
	        dialog = xbmcgui.Dialog()
		links = dialog.input('Adiciona o link do virtual addon:','link', type=xbmcgui.INPUT_ALPHANUM)
		url = links
...
def web_conect():
    		...
    		listaw = os.path.join(xbmc.translatePath("special://home/addons/webinterface.kratos/lista.xml").decode("utf-8"))
                tubew  = os.path.join(xbmc.translatePath("special://home/addons/webinterface.kratos/tube.xml").decode("utf-8"))
    		webw  = os.path.join(xbmc.translatePath("special://home/addons/webinterface.kratos/mag.xml").decode("utf-8"))
    		...
    		shutil.copy2(lista, webi)
    		shutil.copy2(tube, webi)
    		shutil.copy2(mag, webi)
    		
                
    		f.write('\n <title>Kratos Weblistas</title><BR>')
    		f.write('\n <b> ***** Download de Lista Personalizadas do Kratos ***** </b><BR><BR>')
                f.write('\n <a href="lista.xml">Lista Principal</a><BR>')
                f.write('\n <a href="tube.xml">Conteudo Youtube</a><BR>')
                f.write('\n <a href="mag.xml">Conteudo Torrent</a><BR>')
                f.write('\n<BR><BR>')
                f.write('\n <a href="http://kratos-addons-kodish.16mb.com/repo/instal/plugin.video.Blackarrowtorrent/plugin.video.Blackarrowtorrent-3.0.0.zip">Extraia o arquivo coloque as Listas na raiz </a><BR>')
                
    
def autoplay():
                dialog = xbmcgui.Dialog()
		link = dialog.input('[COLOR yellow]Cole link Para Tocar seu Seu Video[/COLOR]','https://pastebin.com/raw/YBzGpgxH', type=xbmcgui.INPUT_ALPHANUM)
		toca = "plugin://plugin.video.playthis/?mode=play&player=false&path="+link+""
...
def autoplayf4m():
		...
		dialog = xbmcgui.Dialog()
		tipo = dialog.input('[COLOR yellow]1 f4m 2 m3u8 3 ts 4 defaul 5 non f4m','1', type=xbmcgui.INPUT_ALPHANUM)
		...
		if tipo == "1":
                ...
                if tipo == "2":
                    f4mt = 'plugin://plugin.video.f4mTester/?url='+link+'&amp;streamtype=HLS'
                    xbmc.Player().play(f4mt)

                if tipo == "3":
                    f4mt = 'plugin://plugin.video.f4mTester/?url='+link+'&amp;streamtype=TSDOWNLOADER&amp;name='+name
                    xbmc.Player().play(f4mt)
...
def addlistaply(titulo,thumb,link):
		f = open(playlistv, 'a')
		f.write('\n <channels>')
		f.write('\n <channel>')
		f.write('\n <name>Lista '+titulo+'</name>')
		f.write('\n <thumbnail>'+thumb+'</thumbnail>')
		f.write('\n <externallink>'+link+'</externallink>')
		f.write('\n <fanart>http://www.seriados.tv.br/wp-content/uploads/2012/08/fdp_23082012_.jpg</fanart>')
		f.write('\n <info></info>')
...
def PlaylisTv_Import():
	        dialog = xbmcgui.Dialog()
		link = dialog.input('Adiciona o playlistv:','playlistv://JsYAAA', type=xbmcgui.INPUT_ALPHANUM)
		keyb = link
		url = keyb
		url=url.replace('playlistv://','http://dougtutoriais.com/playlistv/')
		url_base = urllib.urlopen(url).read()
		link = base64.decodestring(url_base)
		...
		for name,url in match:
			if 'link do grupo' not in name:
				#print(name,url,1,'play' , 'FANART','','','','')
				addlistaply(name,thumb,url)
...
def addyoutube():
                ...
                dialog = xbmcgui.Dialog()
		link = dialog.input('[COLOR yellow]Cole um Video,Canal ou Playlist[/COLOR]','https://pastebin.com/raw/YBzGpgxH', type=xbmcgui.INPUT_ALPHANUM)
...
def addyoutubel():
                ...
                dialog = xbmcgui.Dialog()
		link = dialog.input('[COLOR yellow]Cole uma Playlist do Youtube deseja que seja listada[/COLOR]','https://pastebin.com/raw/YBzGpgxH', type=xbmcgui.INPUT_ALPHANUM)
		titulo = dialog.input('[COLOR yellow]Digite nome do Conteudo[/COLOR]','Youtube Favorito', type=xbmcgui.INPUT_ALPHANUM)
		thumb = dialog.input('[COLOR yellow]Digite Thumbnail da Lista[/COLOR]', '',type=xbmcgui.INPUT_ALPHANUM)
...
def addtorrent():
                ...
                dialog = xbmcgui.Dialog()
		link = dialog.input('[COLOR yellow]Cole magnet ou arquivo torrent aqui[/COLOR]','https://pastebin.com/raw/YBzGpgxH', type=xbmcgui.INPUT_ALPHANUM)
		titulo = dialog.input('[COLOR yellow]COloque nome do conteudo do torrent [/COLOR]','Youtube Favorito', type=xbmcgui.INPUT_ALPHANUM)
		thumb = dialog.input('[COLOR yellow]Digite Thumbnail do Torrent[/COLOR]', '',type=xbmcgui.INPUT_ALPHANUM)
...
def controle_lista():
            addDir('','201',201,icon,FANART,'','','','')
            addDir('[COLOR yellow][B]ADICIONAR CONTEUDO [/COLOR][/B]','201',201,icon,FANART,'','','','')
            addDir('','201',201,icon,FANART,'','','','')
            addDir('[COLOR white][B]ADICIONAR LISTA M3U e XML [/COLOR][/B][COLOR green][B]KRATOS [/COLOR][COLOR yellow]KODI BR[/B][/COLOR]','102',102,icon,FANART,'','','','')
            addDir('[COLOR white][B]ADICIONAR CONTEUDO YOUTUBE [/COLOR][/B][COLOR green][B]KRATOS [/COLOR][COLOR yellow]KODI BR[/B][/COLOR]','111',111,icon,FANART,'','','','')
	    addDir('[COLOR white][B]ADICIONAR UMA PLAYLIST DO YOUTUBE LISTADA [/COLOR][/B][COLOR green][B]KRATOS [/COLOR][COLOR yellow]KODI BR[/B][/COLOR]','118',118,icon,FANART,'','','','')
            addDir('[COLOR white][B]ADICIONAR CONTEUDO TORRENT [/COLOR][/B][COLOR green][B]KRATOS [/COLOR][COLOR yellow]KODI BR[/B][/COLOR]','115',115,icon,FANART,'','','','')
            addDir('[COLOR white][B]ADICIONAR CONTEUDO PLAYLISTV [/COLOR][/B][COLOR green][B]KRATOS [/COLOR][COLOR yellow]KODI BR[/B][/COLOR]','170',170,icon,FANART,'','','','')
            #addDir('[COLOR white][B]PLAYLIST LOADER [/COLOR][/B][COLOR green][B]KRATOS [/COLOR][COLOR yellow]KODI BR[/B][/COLOR]','1001',1001,icon,FANART,'','','','')
            

            addDir('','201',201,icon,FANART,'','','','')
            addDir('[COLOR orange][B]VISUALIZAR CONTEUDO [/COLOR][/B]','201',201,icon,FANART,'','','','')
            addDir('','201',201,icon,FANART,'','','','')
	    addDir('[COLOR white][B]VISUALIZAR LISTA M3U e XML [/COLOR][/B][COLOR green][B]KRATOS [/COLOR][COLOR yellow]KODI BR[/B][/COLOR]','103',103,icon,FANART,'','','','')
	    addDir('[COLOR white][B]VISUALIZAR CONTEUDO DO YOUTUBE [/COLOR][/B][COLOR green][B]KRATOS [/COLOR][COLOR yellow]KODI BR[/B][/COLOR]','112',112,icon,FANART,'','','','')
	    addDir('[COLOR white][B]VISUALIZAR CONTEUDO TORRENT [/COLOR][/B][COLOR green][B]KRATOS [/COLOR][COLOR yellow]KODI BR[/B][/COLOR]','116',116,icon,FANART,'','','','')
            addDir('[COLOR white][B]VISUALIZAR CONTEUDO PLAYLISTV[/COLOR][/B][COLOR green][B]KRATOS [/COLOR][COLOR yellow]KODI BR[/B][/COLOR]','171',171,icon,FANART,'','','','')

            #SKindex5()
	    addDir('','201',201,icon,FANART,'','','','')
            addDir('[COLOR red][B]DELETAR CONTEUDO [/COLOR][/B]','201',201,icon,FANART,'','','','')
            addDir('','201',201,icon,FANART,'','','','')
            addDir('[COLOR white][B]APAGAR A LISTA [/COLOR][/B][COLOR green][B]KRATOS [/COLOR][COLOR yellow]KODI BR[/B][/COLOR]','105',105,icon,FANART,'','','','')
	    addDir('[COLOR white][B]APAGAR CONTUDO DO YOUTUBE[/COLOR][/B][COLOR green][B]KRATOS [/COLOR][COLOR yellow]KODI BR[/B][/COLOR]','113',113,icon,FANART,'','','','')
	    addDir('[COLOR white][B]APAGAR TODOS OS TORRENT [/COLOR][/B][COLOR green][B]KRATOS [/COLOR][COLOR yellow]KODI BR[/B][/COLOR]','117',117,icon,FANART,'','','','')
	    addDir('[COLOR white][B]APAGAR TODOS OS PLAYLISTV [/COLOR][/B][COLOR green][B]KRATOS [/COLOR][COLOR yellow]KODI BR[/B][/COLOR]','172',172,icon,FANART,'','','','')
            addDir('','201',201,icon,FANART,'','','','')
            addDir('[COLOR white][B]REPRODUZIR UM VIDEO[/COLOR][/B]','114',114,icon,FANART,'','','','')
            addDir('[COLOR white][B]REPRODUZIR UM VIDEO F4M TESTER[/COLOR][/B]','173',173,icon,FANART,'','','','')
            addDir('','201',201,icon,FANART,'','','','')
...
base = "aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL2ZsZWNoYW5lZ3JhL2JsYWNrYXJyb3d0b3JyZW50L21hc3Rlci9mb250ZSUyMDElMjB0b3JyZW50"
tam = len(base)
...
if base[0:4] == "aHR0":
    ...
    tp  = os.path.join(xbmc.translatePath("special://home/addons/plugin.video.Blackarrowtorrent/resources/scripts/open.mp3").decode("utf-8"))
    xbmc.Player().play(tp)
...
def SKindex():
    addon_log("SKindex")
    getData(off,'')
    msg2 = "https://raw.githubusercontent.com/flechanegra/blackarrowtorrent/master/alarme.txt"				
    msg = msg2 
            ...
            if len(sources) > 1:
                for i in sources:
                    ...
                    if isinstance(i, list):
                        ...
                        titulo = ""+i['title'].encode('utf-8'),grade.grade(i['grade'].encode('utf-8'))+""
                        addDir(titulo,i['url'].encode('utf-8'),1,thumb,fanart,desc,genre,date,credits,'source')
...
#                print channel
                ...
                try:
                    if linkedUrl=='':
                        ...
                        #print linkedUrl
                        if 'TESTE' in desc:
                            addDir_Serie(name.encode('utf-8'),linkedUrl.encode('utf-8'),1,thumbnail,fanArt,'Sim',genre,date,None)
                        else:
...
def addDir_Serie(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False):
        ...
        if date == '':
            ...
            description += '\n\nDate: %s' %date
        contextMenu = []
        if 'Sim' in description:
            mg = metahandlers.MetaData()
            meta = mg.get_meta('tvshow', name=genre.encode('utf-8') ,year=date.encode('utf-8'))
            liz=xbmcgui.ListItem(meta['title'], iconImage=meta['cover_url'], thumbnailImage=meta['cover_url'])
        ...
        elif 'Nao' in description or not 'Sim' in description:
        ...
        if showcontext:
            ...
            if not name in FAV:
                contextMenu.append(('Add to Add-on Favoritess','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
Rsolvers_Kratos = Player_Kratos.Rsolvers_Kratos
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
		...
		dialog=xbmcgui.Dialog()
		rNo=dialog.select('-=Kratos Kodi Br =- select', List)
		if rNo>=0:
			rName=name
			rURL=str(ListU[rNo])
			...
			if 'magnet:?xt=' in rURL:
				...
				Player_Kratos.Resolvers(link,name,iconimage)	    
                        ...
                        if 'emu://server01on/' in rURL:
                            tam = len(rURL)
                            link = "plugin://plugin.video.playthis/?mode=play&player=false&path=http://v1.ec.cx/RedeCanais/RedeCanais/RCFServer1/ondemand/"+rURL[17:tam]+""
                            xbmc.Player().play(link)

                        if 'emu://server02/' in rURL:
                            tam = len(rURL)
                            link = "plugin://plugin.video.playthis/?mode=play&player=false&path=http://d3nmi1vru110g2.ec.cx/RedeCanais/RedeCanais/RCServer02/ondemand/"+rURL[15:tam]+""
                            xbmc.Player().play(link)

                        if 'emu://server02rf/' in rURL:
                            tam = len(rURL)
                            link = "plugin://plugin.video.playthis/?mode=play&player=false&path=http://d83tbaqcuezmn.ec.cx/RedeCanais/RedeCanais/RCFServer2/ondemand/"+rURL[17:tam]+""
                            xbmc.Player().play(link)

                        if 'emu://server03/' in rURL:
                            tam = len(rURL)
                            link = "plugin://plugin.video.playthis/?mode=play&player=false&path=http://d3eryu5vlst7sb.ec.cx/RedeCanais/RedeCanais/RCFServer3/ondemand1/"+rURL[15:tam]+""
                            xbmc.Player().play(link)

                        if 'emu://server04/' in rURL:
                            tam = len(rURL)
                            link = "plugin://plugin.video.playthis/?mode=play&player=false&path=http://ds5b1da2hd79u.ec.cx/RedeCanais/RCFServer4/ondemand1/"+rURL[15:tam]+""
                            xbmc.Player().play(link)

                        if 'emu://server05/' in rURL:
                            tam = len(rURL)
                            link = "plugin://plugin.video.playthis/?mode=play&player=false&path=http://ds7j93oq6nvmh.ec.cx/RedeCanais/RCServer05/ondemand/"+rURL[15:tam]+""
                            xbmc.Player().play(link)

                        if 'emu://server06/' in rURL:
                            tam = len(rURL)
                            link = "plugin://plugin.video.playthis/?mode=play&player=false&path=http://d3nmi1vru110g2.ec.cx/RedeCanais/RedeCanais/RCServer06/ondemand/"+rURL[15:tam]+""
                            xbmc.Player().play(link)

                        if 'emu://server07/' in rURL:
                            tam = len(rURL)
                            link = "plugin://plugin.video.playthis/?mode=play&player=false&path=http://d3nmi1vru110g2.ec.cx/RedeCanais/RedeCanais/RCServer07/ondemand/"+rURL[15:tam]+""
                            xbmc.Player().play(link)

                        if 'emu://server08/' in rURL:
                            tam = len(rURL)
                            link = "plugin://plugin.video.playthis/?mode=play&player=false&path=http://d18mt2vjbif0ef.ec.cx/RedeCanais/RCServer08/ondemand/"+rURL[15:tam]+""
                            xbmc.Player().play(link)

                        if 'emu://server09/' in rURL:
                            tam = len(rURL)
                            link = "plugin://plugin.video.playthis/?mode=play&player=false&path=http://d3eryu5vlst7sb.ec.cx/RedeCanais/RedeCanais/RCServer09/ondemand/"+rURL[14:tam]+""
                            xbmc.Player().play(link)

                        if 'emu://server10/' in rURL:
                            tam = len(rURL)
                            link = "plugin://plugin.video.playthis/?mode=play&player=false&path=http://d83tbaqcuezmn.ec.cx/RedeCanais/RedeCanais/RCServer10/ondemand/"+rURL[15:tam]+""
                            xbmc.Player().play(link)

                        if 'emu://server11/' in rURL:
                            tam = len(rURL)
                            link = "plugin://plugin.video.playthis/?mode=play&player=false&path=http://d83tbaqcuezmn.ec.cx/RedeCanais/RedeCanais/RCServer11/ondemand/"+rURL[15:tam]+""
                            xbmc.Player().play(link)

                        if 'emu://server12/' in rURL:
                            tam = len(rURL)
                            link = "plugin://plugin.video.playthis/?mode=play&player=false&path=http://d2damwmtevqrz3.ec.cx/RedeCanais/RCServer12/ondemand/"+rURL[15:tam]+""
                            xbmc.Player().play(link)

                        if 'emu://server13/' in rURL:
                            tam = len(rURL)
                            link = "plugin://plugin.video.playthis/?mode=play&player=false&path=http://ds5b1da2hd79u.ec.cx/RedeCanais/RCServer13/ondemand/"+rURL[15:tam]+""
                            xbmc.Player().play(link)

                        if 'emu://playthis/' in rURL:
                            tam = len(rURL)
                            link = "plugin://plugin.video.playthis/?mode=play&player=false&path="+rURL[16:tam]+""
                            xbmc.Player().play(link)

                        if 'emu://netcine17/' in rURL:
                            tam = len(rURL)
                            link = "plugin://plugin.video.playthis/?mode=play&player=false&path=http://cdn1.ntcdn.us/content/httpdelivery/filmes2017/"+rURL[16:tam]+""
                            xbmc.Player().play(link)

                        if 'emu://netcinedev/' in rURL:
                            tam = len(rURL)
                            link = "plugin://plugin.video.playthis/?mode=play&player=false&path=http://cdn1.ntcdn.us/content/httpdelivery/"+rURL[17:tam]+""
                            xbmc.Player().play(link)

                        if 'emu://netcinebucket/' in rURL:
                            tam = len(rURL)
                            link = "plugin://plugin.video.playthis/?mode=play&player=false&path=http://cdn1.ntcdn.us/content/sobdemanda/netcine-bucket/"+rURL[19:tam]+""
                            xbmc.Player().play(link)

                        if 'emu://imdb/' in rURL:
                            tam = len(rURL)
                            link = "plugin://plugin.video.webplayer/?logos=&amp;move=0&amp;mode=3&amp;index=-1&amp;url=http://www.imdb.com/title"+rURL[10:tam]+""
                            xbmc.Player().play(link)

                        if 'srt://' in rURL:
                            tam = len(rURL)
                            link = rURL[6:tam]
                            description = "Legenda Teste"
                            legenda.sublegenda(name,link,description)
...
#                    print 'item link', item('link')
                    ...
                    for i in item('netcine17'):
                        if not i.string == None:
                            netcine17 = 'http://cdn1.ntcdn.us/content/httpdelivery/filmes2017/'+i.string+'.mp4'
                            url.append(netcine17)

                elif len(item('netcinedev')) >0:
                    for i in item('netcinedev'):
                        if not i.string == None:
                            netcinedev = 'http://cdn1.ntcdn.us/content/httpdelivery/'+i.string+'.mp4'
                            url.append(netcinedev)

                elif len(item('netcinebucket')) >0:
                    for i in item('netcinebucket'):
                        if not i.string == None:
                            netcinebucket = 'http://cdn1.ntcdn.us/content/sobdemanda/netcine-bucket/'+i.string+'.mp4'
                            url.append(netcinebucket)


                elif len(item('server01')) >0:
                    for i in item('server01'):
                        if not i.string == None:
                            server01 = 'http://d3eryu5vlst7sb.ec.cx/RedeCanais/RedeCanais/RCServer01/videos/'+i.string+'.mp4|'+ftunel+''
                            url.append(server01)

                elif len(item('server01on')) >0:
                    for i in item('server01'):
                        if not i.string == None:
                            server01 = 'http://v2.ec.cx/RedeCanais/RedeCanais/RCFServer1/ondemand/'+i.string+'.mp4|'+ftunel+''
                            url.append(server01on)

                            

                elif len(item('server03')) >0:
                    for i in item('server03'):
                        if not i.string == None:
                            server03 = 'http://d2damwmtevqrz3.ec.cx/RedeCanais/RCFServer3/ondemand/'+i.string+'.mp4|'+ftunel+''
                            url.append(server03)

                elif len(item('server02')) >0:
                    for i in item('server02'):
                        if not i.string == None:
                            server02 = 'http://d83tbaqcuezmn.ec.cx/RedeCanais/RedeCanais/RCFServer2/ondemand/'+i.string+'.mp4|'+ftunel+''
                            url.append(server02)

                elif len(item('server02srv')) >0:
                    for i in item('server02srv'):
                        if not i.string == None:
                            server02srv = 'http://d3nmi1vru110g2.ec.cx/RedeCanais/RedeCanais/RCServer02/ondemand/'+i.string+'.mp4|'+ftunel+''
                            url.append(server02srv)
                            
                            
                elif len(item('server04')) >0:
                    for i in item('server04'):
                        if not i.string == None:
                            server04 = 'http://d3eryu5vlst7sb.ec.cx/RedeCanais/RedeCanais/RCFServer4/ondemand1/'+i.string+'.mp4|'+ftunel+''
                            url.append(server04)

                elif len(item('server04srv')) >0:
                    for i in item('server04srv'):
                        if not i.string == None:
                            server04srv = 'http://v1.ec.cx/RedeCanais/RedeCanais/RCServer04/ondemand/'+i.string+'.mp4|'+ftunel+''
                            url.append(server04srv)
            

                elif len(item('server05')) >0:
                    for i in item('server05'):
                        if not i.string == None:
                            server05 = 'http://ds7j93oq6nvmh.ec.cx/RedeCanais/RCServer05/ondemand/'+i.string+'.mp4|'+ftunel+''
                            url.append(server05)
							
                elif len(item('server06')) >0:
                    for i in item('server06'):
                        if not i.string == None:
                            server06 = 'http://d3nmi1vru110g2.ec.cx/RedeCanais/RedeCanais/RCServer06/ondemand/'+i.string+'.mp4|'+ftunel+''
                            url.append(server05)

                elif len(item('server07')) >0:
                    for i in item('server07'):
                        if not i.string == None:		    
                            server07 = 'http://d3nmi1vru110g2.ec.cx/RedeCanais/RedeCanais/RCServer07/ondemand/'+i.string+'.mp4|'+ftunel+''
                            url.append(server07)
                    ...
                    for i in item('server08'):
                        if not i.string == None:
                            server08 = 'http://d18mt2vjbif0ef.ec.cx/RedeCanais/RCServer08/ondemand/'+i.string+'.mp4|'+ftunel+''
                            url.append(server08)
                            
                elif len(item('server09')) >0:
                    for i in item('server09'):
                        if not i.string == None:
                            server09 = 'http://d3eryu5vlst7sb.ec.cx/RedeCanais/RedeCanais/RCServer09/ondemand/'+i.string+'.mp4|'+ftunel+''
                            url.append(server09)
                    ...
                    for i in item('server10'):
                        if not i.string == None:
                            server10 = 'http://d83tbaqcuezmn.ec.cx/RedeCanais/RedeCanais/RCServer10/ondemand/'+i.string+'.mp4|'+ftunel+''
                            url.append(server10)

                elif len(item('server11')) >0:
                    for i in item('server11'):
                        if not i.string == None:
                            server11 = 'http://d83tbaqcuezmn.ec.cx/RedeCanais/RedeCanais/RCServer11/ondemand/'+i.string+'.mp4|'+ftunel+''
                            url.append(server11)

                elif len(item('server12')) >0:
                    for i in item('server12'):
                        if not i.string == None:
                            server12 = 'hhttp://d2damwmtevqrz3.ec.cx/RedeCanais/RCServer12/ondemand/'+i.string+'.mp4|'+ftunel+''
                            url.append(server12)
							
		elif len(item('server13')) >0:
                    for i in item('server13'):
                        if not i.string == None:
                            server13 = 'http://ds5b1da2hd79u.ec.cx/RedeCanais/RCServer13/ondemand/'+i.string+'.mp4|'+ftunel+''
                            url.append(server13)
                ...
                elif len(item('trailer')) >0:
                    for i in item('trailer'):
                        if not i.string == None:
                            trailer = ''+i.string+''
                            url.append(trailer)
            ...
            try:
                if len(url) > 1:
                    ...
                    if isXMLSource:
                        ...
                        if "TESTE" in desc:
                            addDir(name.encode('utf-8'),url[0],30,thumbnail,fanArt,'Sim',genre,date,None)
                        else:
                        ...
                        if "TESTE" in desc:					
                            addLink(url[0],name.encode('utf-8', 'ignore'),thumbnail,fanArt,'Sim',genre,date,desc,None,regexs,total)
                        else:
                            addLink(url[0],name.encode('utf-8', 'ignore'),thumbnail,fanArt,desc,genre,date,desc,None,regexs,total)
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False):
        ...
        if 'Sim' in description:
            anos = 'Sim'
            mg = metahandlers.MetaData()
            meta = mg.get_meta('movie', name=genre.encode('utf-8') ,year=date.encode('utf-8'))
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if regexs: 
            ...
            if 'XBMCtorrent' in plugin:
                ...
                mode = '12'
        else:#['pipocao.com','redecanais.','netcine.us','mmfilmes.tv']
            if 'sublink' in url:
                mode = '30'
            else:
                if any(x in url for x in Rsolvers_Kratos) and not '.mp4' in url:
                    mode = '200'
                else:
                    mode = '12'
...
if mode==None:
	...
	Player_Kratos.Check_update()
	Player_Kratos.Resolvers(url,name,iconimage)


elif mode==13:
    addon_log("play_playlist")
    playlist.play_playlist2(name, playlist)
