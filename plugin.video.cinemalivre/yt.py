...
def Scrape(id):
    ...
    video  = {}
    links  = []
    url  = id#'http://www.youtube.com/watch?v=%s&safeSearch=none' % id
    #url = 'https://docs.google.com/file/d/0BwAtu2-H0w2RcmhROExsdDZMYWs/edit'
    urls = []
    names = []
    html = FetchPage(url)
    flashvars = ExtractFlashVars(html)
    itags = {'5': '240', '6': '270', '17': '144', '18': '360', '22': '720', '34': '360', '35': '480',
                         '36': '240', '37': '1080', '38': '3072', '43': '480', '44': '480', '45': '720', '46': '1080',
    ...
    for url_desc in flashvars[u"url_encoded_fmt_stream_map"].split(u","):
      ...
      #  print itags.get(key)+'\nLINK DO VIDEO: \n'+url+'\n\n'#{"NAME": itags.get(key),"LINK": url}#itags.get(key)+'\nLINK DO VIDEO: \n'+url+'\n\n'
        xbmc.log('%s ' % key)
        names.append(itags.get(str(key)))
        urls.append(url)	
    opcao = xbmcgui.Dialog().select('Lojink_resolver_youtube', names)
    if opcao>= 0:
        url = urls[opcao]
        return url
    if opcao:
