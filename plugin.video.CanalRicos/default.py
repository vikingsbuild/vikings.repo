...
addon = xbmcaddon.Addon('plugin.video.CanalRicos')
addonname = ('[COLOR white][B]Canal[/B][CLOR red][B]Ricos[/B][/COLOR]')
...
CHBase = base64.decodestring('aHR0cDovL2JpdC5seS9jYW5hbHJpY29z')

def CHIndex():
    addon_log("CHIndex")
    getData(CHBase,'')
    msg = "https://pastebin.com/raw/uZC7zCZR"
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.CanalRicos-%s]: %s" %(addon_version, string))
...
def SKindex():
    addon_log("SKindex")
    getData(ksd,'')
    msg2 = ("https://pastebin.com/raw/uZC7zCZR")				
    msg = msg2 
    line1 = urllib2.urlopen(msg).read()
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(CanalRicos,Novos Conteúdos Adicionados.,5000,"+icon+")")
        if not url is None:
...
def SearchChannels():
    ...
    progress.create('CanalRicos Procurando Espere',' ')
...
def play_playlist(name, mu_playlist,queueVideo=None):
        ...
        if '$$LSPlayOnlyOne$$' in mu_playlist[0]:
            ...
            for i in mu_playlist:
                ...
                iloop+=1
                dialog = xbmcgui.Dialog()
            index = dialog.select('[COLOR white][B]Br[/B][CLOR red][B]Live[/B][/COLOR] - Selecione uma Opção:', names)
            if index >= 0:
...
#                    print sepate
        ...
        if addon.getSetting('ask_playlist_items') == 'true' and not queueVideo :
            ...
            for i in mu_playlist:
                ...
                iloop+=1
            dialog = xbmcgui.Dialog()
            index = dialog.select('[COLOR white][B]Br[/B][CLOR red][B]Live[/B][/COLOR]- Escolha uma opcao:', names)
            if index >= 0:
                playname=names[index]
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if not name in FAV:
                contextMenu.append(('Add to CanalRicos Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#                print 'setting as folder in link'
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from CanalRicos Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to CanalRicos Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
#                print 'proxy',proxy
            ...
            xbmc.executebuiltin("XBMC.Notification(CanalRicos,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(CanalRicos,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(CanalRicos,Parental Block Disabled,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(CanalRicos,Wrong Pin??,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(CanalRicos,Parental block enabled,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
