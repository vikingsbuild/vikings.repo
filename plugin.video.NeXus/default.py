...
def getItems(items,fanart,dontLink=False):
        ...
        for item in items:
            ...
            try:
                ...
                elif len(item('nexusv1')) >0:
                    for i in item('nexusv1'):
                        if not i.string == None:
                            nexusv1 = 'plugin://plugin.video.gdrive?mode=streamURL&amp;url=https://docs.google.com/file/d/'+i.string
                            url.append(nexusv1)
							
                elif len(item('nexusv2')) >0:
                    for i in item('nexusv2'):
                        if not i.string == None:
                            nexusv2 = 'https://goo.gl/'+i.string
                            url.append(nexusv2)

                elif len(item('nexusv3')) >0:
                    for i in item('nexusv3'):
                        if not i.string == None:
                            nexusv3 = 'https://www.googleapis.com/drive/v3/files/'+i.string
                            url.append(nexusv3)

                elif len(item('nexusv4')) >0:
                    for i in item('nexusv4'):
                        if not i.string == None:
                            nexusv4 = 'https://www.googleapis.com/drive/'+i.string
                            url.append(nexusv4)
