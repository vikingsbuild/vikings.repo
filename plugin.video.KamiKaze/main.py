...
##CONFIGURAÇÕES
####  TITULO DO MENU  #################################################################
title_menu = "[B][COLOR white]BEM-VINDOS AO DOJÔ[COLOR darkorange][B]KamiKaze[/COLOR][/B]"
###  DESCRIÇÃO DO ADDON ###############################################################
title_descricao = "KamiKaze\nTrazendo o melhor conteudo de filmes e séries para você e sua familia"
####  LINK DO TITULO DE MENU  #########################################################
url_title = ''

#### MENSAGEM BEM VINDOS  #############################################################
titulo_boas_vindas = "BEM-VINDOS"
mensagem_bem_vindo = "[B][COLOR white]Obrigado por ter escolhido Fazer parte da Clã KamiKaze![/COLOR]\n[COLOR white]Entre no nosso grupo do:[/COLOR] [COLOR green]Whatsaap[/COLOR] https://chat.whatsapp.com/Kqfc8v6CBJL8nL6KhmZ82Y[/B]"
####  MENSAGEM SECUNDARIA ######################################################
mensagem_secundaria = "[COLOR white][B]Kami[/B][/COLOR][COLOR darkorange][B]Kaze[/B][/COLOR] - [COLOR white][B]O MELHOR DO ENTRETENIMENTO![/B][/COLOR]"
...
thumb_icon_config = 'https://i.imgur.com/yAk7XQZ.jpg'
desc_configuracoes = "Configurações do meu addon"

## FAVORITOS
menu_favoritos = "[B][COLOR white]FAVORITOS[/COLOR][/B]"
thumb_favoritos = 'https://i.imgur.com/q09OJRb.png'
desc_favoritos = "Adicione Itens aos Favoritos, pressionando OK do controle ou clicando o direito e selecionando Adicionar aos favoritos do Meu Addon"

#### MENU VIP ################################################################
titulo_vip = "[B][COLOR white]KamiKaze[/COLOR] - [COLOR gold](VIP)[/COLOR][/B]"
thumbnail_vip = 'https://i.imgur.com/Yc950wi.jpg'
fanart_vip = 'https://i.imgur.com/Yc950wi.jpg'
#### DESCRIÇÃO VIP ###########################################################
vip_descricao = 'SOLICITE SEU TESTE VIP'
#### DIALOGO VIP - SERVIDOR DESATIVADO - CLICK ###################################
vip_dialogo = "[B][COLOR white]CONFIGURE SEU ACESSO A NOSSA ÁREA VIP ANTES DE ENTRAR AQUI, MAIS INFORMAÇÕES ENTRE EM CONTATO COM NOSSO SUPORTE\nENTRE EM COMTATO PELO WHATSAPP: 55+ 11 987751081 E SOLICITE SEU TESTE[/COLOR][/B]"
##SERIVODR VIP
url_server_vip = 'http://psrv.io:80/get.php'

##CONTADOR DE ACESSO
url_contador = 'https://whos.amung.us/pingjs/?k=xghbks619t'
nome_contador = 'MeuAddon-1.0.0'


## CONFIGURAÇÃO DO MULTLINK - VARIAS OPÇÕES POR LINK
...
#Base
url_b64_principal = 'aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL25pZ2h0d29sZmFkZG9uL0thbWlLYXplL21hc3Rlci9DYXRlZ29yaWFz'
...
if sys.argv[1] == 'limparFavoritos':
    ...
    if exists:
        try:
            ...
            pass
    xbmcgui.Dialog().ok('Sucesso', '[B][COLOR white]Favoritos limpo com sucesso![/COLOR][/B]')
    xbmc.sleep(2000)
    exit()
    ...
    for item in items:
        ...
        try:
            if resolver(url, name, thumbnail).startswith('plugin://plugin.video.youtube/playlist') == True or resolver(url, name, thumbnail).startswith('plugin://plugin.video.youtube/channel') == True or resolver(url, name, thumbnail).startswith('plugin://plugin.video.youtube/user') == True or resolver(url, name, thumbnail).startswith('Plugin://plugin.video.youtube/playlist') == True:
                ...
                addDir2(name.encode('utf-8', 'ignore')+'[COLOR aquamarine] ('+str(len(url2))+' itens)[/COLOR]'.encode('utf-8', 'ignore'),str(url2).replace(',','||').replace('$'+playlist_command+'','#'+playlist_command+''),11,str(subtitle2).replace(',','||'),'',thumbnail,fanArt,desc.encode('utf-8'),False)
                ...
                addDir2(name.encode('utf-8', 'ignore')+'[COLOR aquamarine] ('+str(len(url2))+' itens)[/COLOR]'.encode('utf-8', 'ignore'),str(url2).replace(',','||').replace('$'+playlist_command+'','#'+playlist_command+''),11,subtitle,'',thumbnail,fanArt,desc.encode('utf-8'),False)
...
def rc_pro(channel):
    ...
    try:
        regex_file = base64.b64decode('aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L0RMQXdmWFF0').decode('utf-8')
        regex = getRequest2(regex_file,'').replace('\n','').replace('\r','')
        match_data = re.compile('popup_referer="(.+?)".+?layer_referer="(.+?)".+?layer="(.+?)"').findall(regex)
        if match_data !=[]:
            ref1 = match_data[0][0]
            ref2 = match_data[0][1]
            player = match_data[0][2]
        else:
            ref1 = 'https://noticiasfix.com/'
            ref2 = result_host
            player = 'player3/canais-bk.php'
        #print(player)
        channel_rc = re.compile("canal=(.*)", re.MULTILINE|re.DOTALL|re.IGNORECASE).findall(channel)
        if channel_rc !=[]:
            data = getRequest2(result_host+player+'?canal='+channel_rc[0], ref1)
        else:
            data = ''
        #print(data)
        if data and data > '':
            ...
            if channel_rc !=[]:
                webdata = getRequest2(ref2+player+'?canal='+channel_rc[0], ref1)
            else:
                webdata = ''
        hostrc = re.compile('source.+?src="(.+?)"', re.MULTILINE|re.DOTALL|re.IGNORECASE).findall(webdata)
        if hostrc !=[]:
            link = hostrc[0].replace('\n','').replace('\r','')
        else:
            link = ''
        if ref2 and channel_rc !=[]:
            channelid = channel_rc[0].replace('\n', '\r')
            rc_ref = ref2+player+'?canal='+urllib.quote_plus(channelid)
            #rc_ref = ref2+player+'?canal='+channelid
        else:
            rc_ref = ''
        if link and link > '' and rc_ref and rc_ref > '':
            link_rc = link
            link_rc_ref = rc_ref
        else:
            link_rc = ''
...
def resolver(link, name, thumbnail):
    ...
    try:
        ...
        if not link_decoded.startswith("plugin://plugin") and link_decoded.startswith('https://drive.google.com') == True:
            ...
            #print(resolved)
        #Rede Canais
        elif not link_decoded.startswith("plugin://plugin") and link_decoded.find('redecanais') >= 0:
            try:
                link_corretor = link_decoded.replace(';','')
                link_rc, link_rc_ref = rc_pro(link_corretor)
                if link_rc and link_rc > '' and link_rc_ref and link_rc_ref > '':                    
                    link_final2 = link_rc+'|Referer='+link_rc_ref+'|User-Agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.88 Safari/537.36'
                    #print(link_final2)
                else:
                    link_final2 = ''
                    #print(link_final2)
...
def addFavorite(name,url,fav_mode,subtitle,cleaname,iconimage,fanart,description):
    ...
    if os.path.exists(favorites)==False:
        ...
        a.close()
        notify('Adicionado aos Favoritos do Meu Addon!')
...
def rmFavorite(name):
    ...
    for index in range(len(data)):
        if data[index][0]==name:
            ...
            b.close()
            break
    notify('Removido dos Favoritos do Meu Addon!')
    xbmc.executebuiltin("XBMC.Container.Refresh")            
...
def addDir(name,url,mode,iconimage,fanart,description,folder=True):
    ...
    if favoritos == 'true' and  mode !=4 and mode !=7 and mode !=8 and mode !=9 and mode !=10 and mode !=15 and not url.find('username') >= 0:
        ...
        try:
            ...
            if name_fav in FAV:
                try:
                    contextMenu.append(('Remover dos Favoritos do Meu Addon','XBMC.RunPlugin(%s?mode=14&name=%s)'%(sys.argv[0], urllib.quote_plus(name))))
                except:
                    contextMenu.append(('Remover dos Favoritos do Meu Addon','XBMC.RunPlugin(%s?mode=14&name=%s)'%(sys.argv[0], urllib.quote_plus(name.encode('utf-8', 'ignore')))))
            else:
                try:            
                ...
                contextMenu.append(('Adicionar aos Favoritos do Meu Addon','XBMC.RunPlugin(%s)' %fav_params))
...
def addLink(name,url,subtitle,cleaname,iconimage,fanart,description,folder=False):
    ...
    if favoritos == 'true':
        try:
            ...
            if name_fav in FAV:
                try:
                    contextMenu.append(('Remover dos Favoritos do Meu Addon','XBMC.RunPlugin(%s?mode=14&name=%s)'%(sys.argv[0], urllib.quote_plus(name2_fav))))
                except:
                    contextMenu.append(('Remover dos Favoritos do Meu Addon','XBMC.RunPlugin(%s?mode=14&name=%s)'%(sys.argv[0], urllib.quote_plus(name2_fav.encode('utf-8', 'ignore')))))
            else:
                try:           
...
#Link Vazio
    ...
    if addon.getSetting('youtubeexterno') == 'true':
        youtube(url)
    else:
